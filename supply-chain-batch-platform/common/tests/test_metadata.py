...
def test_inmemory_file_dedup():
    store = InMemoryMetadataStore()
    store.write_file(FileAudit(batch_id="b1", source="sap", filename="po.csv",
                               checksum="abc", size_bytes=10))
...
def test_jsonl_store_persists_and_dedups(tmp_path):
    ...
    store.write_file(FileAudit(batch_id="b1", source="tms", filename="ship.parquet",
                               checksum="xyz", size_bytes=99))
    store.write_dq([DQResultRecord(batch_id="b1", entity="shipment", rule="NotNull(id)",
                                   severity="error", passed=9, failed=1, threshold=0.0,
                                   breached=True, sample_keys=["S9"])])
