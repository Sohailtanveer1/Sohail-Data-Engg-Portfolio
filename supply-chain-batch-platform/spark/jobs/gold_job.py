...
def _dims_from_cfg(cfg: dict) -> list[DimJoin]:
    return [
        DimJoin(
            name=d["name"],
            table=d["table"].replace("scb.", ""),
            business_key=d["business_key"],
            sk_column=d["sk"],
            scd=d.get("scd", "scd1"),
            fact_date=d.get("fact_date"),
...
def build_fact(spark, cfg: dict, *, catalog: str):
    ...
    sql = build_fact_select(
        ...
        dims=[
            DimJoin(**{**d.__dict__, "table": d.table.replace("scb.", f"{catalog}.")})
...
def write_bigquery(
    df, *, table: str, dataset: str, project: str, temp_bucket: str, partition_field: str | None
) -> None:
    writer = (
...
def run_gold(
    ...
    *,
    ...
    entity = cfg["entity"]
    ctx = BatchContext(pipeline=f"scb_{entity}_gold", source="silver", destination="gold", env=env)
    log = get_logger(f"gold.{entity}", ctx=ctx)
    audit = BatchAudit(
        batch_id=ctx.batch_id, pipeline=ctx.pipeline, source="silver", destination="gold"
    )
    try:
        ...
        write_bigquery(
            ...
            partition_field=cfg.get("date_column"),
        )
        ...
        log.metrics(
            status="success", rows_read=rows, rows_written=rows, duration_s=audit.duration_s
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    spark = build_spark(
        app_name=f"scb-gold-{args.entity}", catalog=args.catalog, warehouse=args.warehouse
    )
    try:
        run_gold(
            spark,
