...
SCD_COLUMNS = [
    ...
    ("is_current", "boolean"), ("row_hash", "string"), ("batch_id", "string"),
]


def _ensure_scd2_table(spark, table: str, sk: str, business_keys: list[str],
                       tracked: list[str], schema: TableSchema) -> list[str]:
    col_types = {c.name: SPARK_TYPES[c.dtype] for c in schema.columns}
    cols = [(sk, "string")] + [(k, col_types[k]) for k in business_keys] \
        + [(t, col_types[t]) for t in tracked] + SCD_COLUMNS
    spark.sql(build_create_iceberg_table(table, cols))
    return [c for c, _ in cols]


def run_silver(spark, entity_cfg: dict, *, bronze_root: str, catalog: str,
               metastore, quarantine_root: str, env: str = "local") -> None:
    ...
    ctx = BatchContext(pipeline=f"scb_{entity}_silver", source=source,
                       destination="silver", env=env)
    log = get_logger(f"silver.{entity}", ctx=ctx)
    audit = BatchAudit(batch_id=ctx.batch_id, pipeline=ctx.pipeline,
                       source=source, destination="silver")
    ...
    try:
        ...
        if q_count:
            (quarantine.write.mode("append")
             .parquet(f"{quarantine_root}/{entity}/batch_id={ctx.batch_id}"))

        from pyspark.sql import functions as F  # local import (Spark-only path)

        if scd == "scd2":
            prepared = (add_row_hash(clean.select(*business_keys, *tracked), tracked)
            ...
            apply_scd2(spark, prepared.select(*insert_cols), target=target,
                       business_keys=business_keys, insert_columns=insert_cols)
            ...
            col_types = [(c.name, SPARK_TYPES[c.dtype]) for c in schema.columns] + [("batch_id", "string")]
            spark.sql(build_create_iceberg_table(target, col_types))
            update_cols = [c for c in all_cols if c not in business_keys]
            apply_scd1(spark, prepared.select(*all_cols), target=target,
                       business_keys=business_keys, update_columns=update_cols,
        ...
        metastore.write_dq([
            DQResultRecord(batch_id=ctx.batch_id, entity=entity, rule=r.rule,
        ...
        log.metrics(status="success", rows_read=rows_read, rows_written=rows_written,
                    rows_rejected=q_count, duration_s=audit.duration_s)
...
def main(argv: list[str] | None = None) -> int:
    ...
    spark = build_spark(app_name=f"scb-silver-{args.entity}", catalog=args.catalog,
                        warehouse=args.warehouse)
    try:
        run_silver(spark, cfg, bronze_root=args.bronze_root, catalog=args.catalog,
